        ...
        def get(self,request):
            ...
            if user.is_authenticated:
                if Rol.objects.filter(user=user, id=3):
                    ...
                    return render(request,"dashcurador.html",{'actions': actions, 'especimenes':especimenes, 'clase':clase_genero})
                else:
                    return HttpResponseRedirect(reverse('redirect'))
...
def export_data_cur(request,clase):
    ...
    data = especimen.objects.filter(ClaseE=clase).values('NumeroCatalogo', 'NombreDelConjuntoDatos', 'ComentarioRegistroBiologico', 'RegistradoPor', 'NumeroIndividuo',
                                       'FechaEvento', 'Habitad', 'Departamento', 'Municipio', 'IdentificadoPor', 'FechaIdentificacion',
                                       'IdentificacionReferencias', 'ComentarioIdentificacion', 'NombreCientificoComentarioRegistroBiologico', 'ClaseE',
                                       'Orden', 'Genero', 'Familia', 'NombreComun')
    data = list(data)  
